module ActivityLogsHelper
  ...
  def activity_to_s(activity)
    ...
    if activity.task_id
      return link_to(activity.developer.name, activity.developer) + " <b>".html_safe + activity.activity_type.to_s + " the task</b> ".html_safe + link_to(activity.task.description, activity.task) + " in " + link_to(activity.project.name, activity.project)
    end
